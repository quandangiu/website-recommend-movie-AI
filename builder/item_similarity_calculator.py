...
class ItemSimilarityMatrixBuilder(object):
    ...
    def __init__(self, min_overlap=2, min_sim=0.1):
    ...
    def build(self, ratings, save=True):
        ...
        # Tính toán sự chồng chéo giữa các mục.
...
def main():
    ...
    ItemSimilarityMatrixBuilder(min_overlap=4, min_sim=0.1).build(all_ratings)

def normalize(x):
    """Chuẩn hóa dữ liệu với xử lý lỗi tốt hơn"""
    x = x.astype(float)
    x_sum = x.sum()
    x_num = x.astype(bool).sum()
    
    # Kiểm tra nếu không có giá trị nào trong x
    if x_num == 0:
        return np.zeros_like(x)  # Trả về mảng toàn số 0
        
    x_mean = x_sum / x_num

    # Xử lý trường hợp đặc biệt để tránh NaN
    if x_num <= 1 or x.std() == 0 or np.max(x) == np.min(x):
        return np.zeros_like(x)  # Trả về mảng toàn số 0
    
    # Xử lý chuẩn hóa an toàn
    normalized = (x - x_mean) / (np.max(x) - np.min(x))
    
    # Thay thế bất kỳ giá trị NaN nào bằng 0
